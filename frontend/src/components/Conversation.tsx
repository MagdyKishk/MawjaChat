...
export default function Conversation({id, name, lastMessage, lastSeen, avatar}: ConversationProps) {
    const {openChat} = useChatStore()
    
    return (
        <Link onClick={() => openChat()} to={`/messages/${id}`} className="flex items-center gap-2 hover:bg-neutral-100 p-2 rounded-lg cursor-pointer">
